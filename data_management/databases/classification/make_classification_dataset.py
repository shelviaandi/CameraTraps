...
import data_management.tfrecords.utils.create_tfrecords_py3 as tfr
...
# test_locations can be empty
test_locations = sorted(random.sample(sorted(locations), int(TEST_FRACTION * len(locations))))
...
with graph.as_default():
  with tf.Session() as sess:
    ...
    for cur_image_id in tqdm.tqdm(list(sorted([vv['id'] for vv in coco.imgs.values()]))):
      ...
      cur_image_id_det = 'SER/' + cur_file_name
      if cur_image_id_det in detections.keys():
        output_dict = detections[cur_image_id_det]
      # Otherwise run detector
      else:
        print('cur_image_id not in existing detections: {}'.format(cur_image_id))
